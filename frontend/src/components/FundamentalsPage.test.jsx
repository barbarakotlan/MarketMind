...
vi.mock('../config/api', () => ({
    API_ENDPOINTS: {
        FUNDAMENTALS: vi.fn((ticker, market = 'US') => `/fundamentals/${ticker}?market=${String(market).toLowerCase()}`),
        FUNDAMENTALS_FINANCIALS: vi.fn((ticker) => `/fundamentals/financials/${ticker}`),
        FUNDAMENTALS_FILINGS: vi.fn((ticker, market = 'US') => `/fundamentals/filings/${ticker}?market=${String(market).toLowerCase()}`),
        FUNDAMENTALS_SEC_INTELLIGENCE: vi.fn((ticker, market = 'US') => `/fundamentals/sec-intelligence/${ticker}?market=${String(market).toLowerCase()}`),
        FUNDAMENTALS_FILING_DETAIL: vi.fn((ticker, accessionNumber) => `/fundamentals/filings/${ticker}/${accessionNumber}`),
    },
    apiRequest: vi.fn(),
}));
